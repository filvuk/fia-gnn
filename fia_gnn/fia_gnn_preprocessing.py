...
def get_output_signature(nfp_preprocessor):
    """Define tensorflow ouput signatures."""
    output_signature = {
...
class fiaGnnPreprocessor:
    ...
    def __init__(
        self, mol, fia_gas_preprocessor=None, fia_solv_preprocessor=None, ca_idx=None
    ) -> None:
    ...
    def get_ca_idx(self):
        ...
        if len(ca_idx) > 1:
            raise ValueError(
                "More than one cantral atom found. Please specify index of central atom."
            )
        ca_idx = ca_idx[0][0]
        return ca_idx
