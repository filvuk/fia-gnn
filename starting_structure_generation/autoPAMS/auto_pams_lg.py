...
def load_building_blocks(type_):
    '''
    Load the building scope which should be used to build the ligands.
    ...
    if type_ == "fully_reduced":
        ...
        sub_2 = "substituents_2_full.json"
        sub_1234 = "substituents_1234_full.json"
        print("[auto_pams_lg] Invalide building block type specified. Defaulting to the full building block space ...")
    ...
    if all([
        ...
        print(f"    [auto_pams_lg] {count(substituents_1234)} general substituents were loaded.")
        print(f"    [auto_pams_lg] {count(substituents_1000)} aromatic ligand substituents were loaded.")
        print(f"    [auto_pams_lg] {count(substituents_2)} aromatic ring substituents were loaded.")
    print("[auto_pams_lg] Building block were successfully loaded. Structures can now be generated.")
        ...
        if atom.GetSymbol() == "*":
            ...
            if len(map_num) not in spare_wildcards:
                ...
                if map_num not in building_blocks["subs"]:
                    ...
                    building_blocks["subs"][map_num]["binding_atom_indices"].append(binding_atom_idx)
                    building_blocks["subs"][map_num]["binding_atom_symbols"].append(binding_atom_symbol)
...
def check_bond_rejection(forbidden_bonds, sub, map_num, building_blocks):
    ...
    for atom in sub.GetAtoms():
        if str(atom.GetAtomMapNum()) == map_num:
            ...
            pot_binding_atom_symbol = sub.GetAtomWithIdx(pot_binding_atom_idx).GetSymbol()
            pot_bonds = [sorted([i, pot_binding_atom_symbol]) for i in building_blocks["subs"][map_num]["binding_atom_symbols"]]
            for b in pot_bonds:
    ...
    for map_num in building_blocks["subs"]:
        if len(map_num) == 2:
            ...
            while reject:
                ...
                reject = check_bond_rejection(forbidden_bonds, sub, map_num, building_blocks)
            if "*:1000" in sub_smi:
                subclass_A = random.choice(list(substituents_1000.keys()))
                subsub_A = Chem.MolFromSmiles(random.choice(substituents_1000[subclass_A]))
            ...
            if map_num.endswith("2"):
                building_blocks["subs"][map_num]["bond_order"] = Chem.rdchem.BondType.DOUBLE
            else:
            ...
            while reject:
                ...
                sub = Chem.MolFromSmiles(random.choice(caps[map_num][subclass][subsubclass]))
                reject = check_bond_rejection(forbidden_bonds, sub, map_num, building_blocks)
...
def assemble(building_blocks, single_sub_only):
    ...
    for map_num in building_blocks["subs"]:
        while isinstance(building_blocks["subs"][map_num]["sub"], list):
            ...
            if len(building_blocks["subs"][map_num]["sub"]) == 2:
                building_blocks["subs"][map_num]["sub"] = assemble(pre_building_blocks, single_sub_only=False)["final"]
            else:
    ...
    for map_num in building_blocks["subs"]:
        ...
        for atom in final.GetAtoms():
            if str(atom.GetAtomMapNum()) == map_num:
                core_binding_indices.append([n.GetIdx() for n in atom.GetNeighbors()][0])
        ...
        for bond in sub.GetBonds():
            if sub_wc_index in bond.GetStereoAtoms():
                sub_stereo_adjustment = (bond.GetIdx(), list(bond.GetStereoAtoms()).index(sub_wc_index))
                break
            else:
        ...
        for idx, start_idx in enumerate(core_binding_indices):
            ...
            if bond_order == Chem.rdchem.BondType.DOUBLE:
                ...
                begin_atom_neighbor_list = [n.GetIdx() for n in final.GetAtomWithIdx(start_idx).GetNeighbors() if n.GetSymbol() != "*" and n.GetIdx() != end_idx]
                if begin_atom_neighbor_list:
                    begin_atom_neighbor = begin_atom_neighbor_list[0]
                else:
                    begin_atom_neighbor_list = [n.GetIdx() for n in final.GetAtomWithIdx(start_idx).GetNeighbors() if n.GetIdx() != end_idx]
                    begin_atom_neighbor = begin_atom_neighbor_list[0]
                # getend_atom_neighbor:
                end_atom_neighbor_list = [n.GetIdx() for n in final.GetAtomWithIdx(end_idx).GetNeighbors() if n.GetSymbol() != "*" and n.GetIdx() != start_idx]
                if end_atom_neighbor_list:
                    end_atom_neighbor = end_atom_neighbor_list[0]
                else:
                    end_atom_neighbor_list = [n.GetIdx() for n in final.GetAtomWithIdx(end_idx).GetNeighbors() if n.GetIdx() != start_idx]
                    end_atom_neighbor = end_atom_neighbor_list[0]
                ...
                if sub_stereo_adjustment:
                    stereo_bond = final.GetBondWithIdx(sub_stereo_adjustment[0] + init_bond_num)
                    stereo_atoms = list(stereo_bond.GetStereoAtoms())
    ...
    if all([
        ...
        while True:
            ...
            if ligand_class == "mono":
                ...
                ligand_subsubclass = random.choice(list(cores[ligand_class][ligand_subclass].keys()))
                ligand_subsubsubclass = random.choice(list(cores[ligand_class][ligand_subclass][ligand_subsubclass].keys()))
                core_smiles = random.choice(cores[ligand_class][ligand_subclass][ligand_subsubclass][ligand_subsubsubclass])
            else:
                ligand_subsubclass = random.choice(list(cores[ligand_class][ligand_subclass].keys()))
                ligand_subsubsubclass = random.choice(list(cores[ligand_class][ligand_subclass][ligand_subsubclass].keys()))
                core_smiles = random.choice(cores[ligand_class][ligand_subclass][ligand_subsubclass][ligand_subsubsubclass])
            ...
            while not done:
                ...
                building_blocks = get_substituents(building_blocks)
                final_ligand = assemble(building_blocks, single_sub_only=random.choice([True, False]))["final"]
        ...
        final_ligand = Chem.RemoveHs(final_ligand) # a single H-atom can be a building block, leading to H-atom objects in mol object
        smiles = Chem.MolToSmiles(final_ligand)
        smiles = Chem.MolToSmiles(Chem.MolFromSmiles(smiles)) # avoid non-canonical smiles
