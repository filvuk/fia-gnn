...
class DataSetManager:
    ...
    gen_structure: callable
        Function which generates the structure of a complex. Returns smiles if structure generation was successful, else returns None.
    only_SMILES: bool, optional
        If the only_SMILES is set False, complexes are not embedded (default True).
    ...
    donor_dict: dict
        ...
        Is generated by calling gen_complex().
    only_SMILES: bool
        If the only_SMILES is set False, complexes are not embedded.
    only_LA: bool
        If the only_LA is set to True, fluoride adducts are generated additionally.
    ...
    load(file_name)
        ...
        gen_ligand: callable
            Function which returns smiles string of a ligand.
        gen_structure: callable
            Function which generates the structure of a complex. Returns smiles if structure generation was successful, else returns False.
        num: int, optional
                ...
                for sdc in self.num_dict[ca][dc]:
                    ...
                    for lc, num in self.num_dict[ca][dc][sdc].items():
                        ...
                        for _ in range(num):
                            ...
                            # gen complex:
                            mol_complex = {}
                            mol_complex["central_atom"] = ca
                            mol_complex["denticity_class"] = dc
                            mol_complex["sub_denticity_class"] = sdc
                            mol_complex["ligand_class"] = lc
                            while True:
                            ...
                            # add complex to data:
                            mol_complex["ligands"] = ligands
                            mol_complex["donors"] = donors
                            mol_complex["smiles"] = smiles
                            mol_complex["smiles__F"] = smiles__F
                            mol_complex["fingerprint"] = fingerprint
                            self.data[file_name] = mol_complex
                            # update donor_dict
    ...
    def save(self, name: str):
        ...
        print("[auto_pams_dm] Save successful")
        print()
        return
    ...
    def get_donors(ligand: str, expected_num: int = 0) -> dict:
        ...
        for atom in mol.GetAtoms():
            if atom.GetNumRadicalElectrons() == 1:
                donors.append(atom)
        # gen symbol for each donor; C and N are further differentiated by hybridization type
